...
interface UnixTimestampCountdownProps {
  endTimestamp: number; // Unix timestamp in seconds
  className?: string;
}

const UnixTimestampCountdown: React.FC<UnixTimestampCountdownProps> = ({ endTimestamp, className }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      // Convert endTimestamp to milliseconds (as if it were milliseconds)
      // This simulates the previous bug where endTimestamp was divided by 1000 again
      const difference = Math.floor(endTimestamp / 1000) - now;
...
export default UnixTimestampCountdown; 