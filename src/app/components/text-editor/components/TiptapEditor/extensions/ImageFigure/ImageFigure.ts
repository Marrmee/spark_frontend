...
// Remove the problematic import and implement a manual clipboard serialization
  ...
  addProseMirrorPlugins() {
    ...
    return [
      new Plugin({
        props: {
          handleDOMEvents: {
            dragstart: (view: EditorView, event) => {
              if (
                !event.dataTransfer ||
                !event.target ||
              ...
              // Use the view's serializeForClipboard method
              const { dom, text } = view.serializeForClipboard(draggedSlice);

              event.dataTransfer.clearData();
              event.dataTransfer.setData("text/html", dom.innerHTML);
              event.dataTransfer.setData("text/plain", text);
              event.dataTransfer.effectAllowed = "copyMove";
              view.dragging = { slice: draggedSlice, move: event.ctrlKey };
