/**
 * Spark Platform - Platform NDA Signature Component
 * 
 * Handles the signing process for the Platform NDA agreement.
...
interface PlatformNDASignatureProps {
  onSuccess?: () => void;
  onCancel?: () => void;
}

const PlatformNDASignature: React.FC<PlatformNDASignatureProps> = ({
  onSuccess,
  onCancel
}) => {
  ...
  const loadNDAContent = async () => {
    try {
      // Load Platform NDA content
      const content = `
SPARK PLATFORM NON-DISCLOSURE AGREEMENT

This Non-Disclosure Agreement ("Agreement") is entered into by and between PoSciDonDAO ("Company") and the user ("Recipient") accessing the Spark platform.

1. CONFIDENTIAL INFORMATION
The Spark platform contains confidential and proprietary information including but not limited to:
- Research ideas and concepts submitted by ideators
- Technical specifications and platform architecture
- User data and analytics (anonymized)
- Platform functionality and proprietary algorithms
- Research methodologies and evaluation criteria
- Commercial partnerships and funding information

2. PURPOSE OF DISCLOSURE
Information is disclosed to enable:
- Platform participation and feature access
- Idea evaluation and peer review processes
- Due diligence and funding decisions
- Platform governance participation

3. OBLIGATIONS
Recipient agrees to:
- Maintain strict confidentiality of all platform information
- Use information solely for authorized platform participation
- Not disclose confidential information to third parties
- Not reproduce, distribute, or share platform data
- Comply with all platform terms and conditions
- Report any unauthorized disclosure or security breaches

4. RESTRICTIONS
Recipient shall NOT:
- Use confidential information for competing platforms
- Share login credentials or access tokens
- Attempt to reverse engineer platform functionality
- Extract or scrape platform data for external use

5. TERM AND TERMINATION
- Agreement effective upon DocuSign completion
- Remains in effect while accessing the Spark platform
- Obligations survive termination of platform access
- Company may terminate access for violations

6. LEGAL COMPLIANCE
This agreement is legally binding and enforceable under applicable laws and blockchain regulations.
...
IMPORTANT: Completion requires DocuSign signature followed by on-chain attestation.
      ...
      throw new Error('Unable to load NDA content');
    }
  };
  ...
  const handleStartDocuSign = async () => {
    ...
    try {
      ...
      // Call DocuSign API endpoint for Platform NDA
  ...
  const handleDocuSignComplete = async (docusignSignature: string) => {
    try {
      ...
      const attestationResponse = await fetch('/api/attestation/platform-nda', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      case 'loading':
        return (
          <div className="flex flex-col items-center justify-center p-8">
            ...
            <p className="text-gray-600">Loading platform NDA...</p>
          </div>
        );
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-blue-900">
                      Spark Platform Access
                    </h3>
                    <p className="text-sm text-blue-700 mt-1">
                      Standard platform NDA for accessing confidential research content and platform features.
                    </p>
                    <div className="text-xs text-blue-600 mt-2 space-y-1">
                      <p>Agreement Type: Platform NDA</p>
                      <p>Scope: Full platform access</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      case 'complete':
        return (
          <div className="flex flex-col items-center justify-center p-8">
            ...
            <h3 className="text-lg font-semibold text-green-900 mb-2">Platform NDA Signed Successfully</h3>
            <p className="text-green-700 text-center">
              You now have full access to the Spark platform and confidential research content.
            </p>
          </div>
        );
    }
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full mx-4 max-h-screen overflow-hidden">
        ...
        <div className="border-b border-gray-200 p-6">
          ...
          <p className="text-sm text-gray-600 mt-2">
            Sign this NDA to access Spark platform features and confidential research content.
          </p>
        </div>
...
export default PlatformNDASignature; 